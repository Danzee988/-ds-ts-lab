...
//   console.log("Contact buddy at: ", findBuddyContact(myBandBuddies, "Ralph Graham"));

function getBuddyListFriends(list: BuddyList): Friend[] {
    return list.members.reduce((friendsArray: Friend[], buddy: Buddy) => {
      const isFriend = friends.some(friend => friend.name === buddy.name);

      if (isFriend) {
        friendsArray.push(buddy as Friend);
      }
      return friendsArray; // Accumulate the friends array
    }, []);
  }
